...
void access_data(mem_addr_t addr) {      
	...
	// find line to change
	for(int i =0; i < E; i++){
		if(cache[set][i].lru < cache[set][line].lru)
			line = i;
	}

	// update values
	cache[set][line].lru = lru_counter;
